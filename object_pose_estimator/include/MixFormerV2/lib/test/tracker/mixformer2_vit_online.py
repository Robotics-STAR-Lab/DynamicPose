...
class MixFormerOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # print(f"Loading checkpoint from {self.params.checkpoint}")
    ...
    def track(self, image, info: dict = None):
        ...
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image, self.state, self.params.search_factor,
                                                                output_sz=self.params.search_size)  # (x1, y1, w, h) 要改sample
    ...
    def update_template(self, image, bbox):
        ...
        z_patch_arr, _, z_amask_arr = sample_target(image, bbox,
                                                    self.params.template_factor,
                                                    output_sz=self.params.template_size)  # (x1, y1, w, h)
        self.online_max_template = self.preprocessor.process(z_patch_arr)

            
        # if self.frame_id % self.update_interval == 0: ## update every interval
        if True: ## update every frame
            # print(f"frame{self.frame_id}: update")
            if self.online_size == 1:
                self.online_template = self.online_max_template
                if self.params.vis_attn == 1:
