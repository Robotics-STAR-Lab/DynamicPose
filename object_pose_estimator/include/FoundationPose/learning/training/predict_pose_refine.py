...
class PoseRefinePredictor:
  def __init__(self, useTRT=False):
  ...
  @torch.inference_mode()
  def predict_pyramid(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=[1]):
    '''
    @rgb: np array (H,W,3)
    @ob_in_cams: np array (N,4,4)
    '''
    ...
    if get_vis:
      ...
      ob_centered_in_cams = ob_in_cams.data.cpu().numpy()
    else:
      ob_centered_in_cams = ob_in_cams
    mesh_centered = mesh
    logging.info(f'self.cfg.use_normal:{self.cfg.use_normal}')
    ...
    if not isinstance(trans_normalizer, float):
      trans_normalizer = torch.as_tensor(list(trans_normalizer), device='cuda', dtype=torch.float).reshape(1,3)
    print(f'\033[31m [predict_pyramid] prepare time={time.time()-start_t}\033[0m')
    
    print(f'\033[34m [before refine] ob_in_cams.shape={ob_in_cams.shape}\033[0m')
    vis_s = []
    start_t = time.time()
    for iter_idx, iter_num in enumerate(iteration):
      logging.info("making cropped data")
      t = time.time()
      pose_data = make_crop_data_batch(self.cfg.input_resize, B_in_cams, mesh_centered, rgb_tensor, depth_tensor, K, crop_ratio=crop_ratio, normal_map=normal_map, xyz_map=xyz_map_tensor, cfg=self.cfg, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, mesh_diameter=mesh_diameter)
      print(f'\033[34m [predict_pyramid] data preparation time={time.time()-t}\033[0m')
      B_in_cams = []
      all_rot_mat_delta = []
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        A = torch.cat([pose_data.rgbAs[b:b+bs].cuda(), pose_data.xyz_mapAs[b:b+bs].cuda()], dim=1).float()
        ...
        if self.cfg['rot_rep']=='axis_angle':
          rot_mat_delta = torch.tanh(output["rot"])*self.cfg['rot_normalizer']
          # for item in rot_mat_delta:
          #   all_rot_mat_delta.append(torch.norm(item))
          all_rot_mat_delta.append(rot_mat_delta)
          rot_mat_delta = so3_exp_map(rot_mat_delta).permute(0,2,1)
        elif self.cfg['rot_rep']=='6d':
          rot_mat_delta = rotation_6d_to_matrix(output['rot']).permute(0,2,1)
        else:
        ...
        B_in_cams.append(B_in_cam)
      if iter_idx == 0:
        reshape_0 = len(ob_in_cams)
      else:
        reshape_0 = iteration[iter_idx-1]
      B_in_cams = torch.cat(B_in_cams, dim=0).reshape(reshape_0,4,4)
      print(f'\033[34m [predict_pyramid] predict time={time.time()-t}\033[0m')
      if get_vis:
        logging.info("get_vis...")
        # print(f'ob_centered_in_cams[0] = \n{ob_centered_in_cams[0]}; \nB_in_cams[0] = \n{B_in_cams[0]}')
        canvas = []
        padding = 2
        pose_data = make_crop_data_batch(self.cfg.input_resize, torch.as_tensor(ob_centered_in_cams), mesh_centered, rgb, depth, K, crop_ratio=crop_ratio, normal_map=normal_map, xyz_map=xyz_map_tensor, cfg=self.cfg, glctx=glctx, mesh_tensors=mesh_tensors, dataset=self.dataset, mesh_diameter=mesh_diameter)
        for id in range(0, len(B_in_cams)):
        ...
        canvas_refined = make_grid_image(canvas_refined, nrow=1, padding=padding, pad_value=255)
        print(f"B_in_cams.shape={B_in_cams.shape}; ob_centered_in_cams.shape={ob_centered_in_cams.shape}")
        canvas = make_grid_image([canvas, canvas_refined], nrow=2, padding=padding, pad_value=255)
        # torch.cuda.empty_cache()
        vis_s.append(canvas)
      print(f'\033[34m [predict_pyramid] get vis time={time.time()-t}\033[0m')
      # all_rot_mat_delta = torch.tensor(all_rot_mat_delta)
      all_rot_mat_delta = torch.cat(all_rot_mat_delta, dim=0)
      all_rot_mat_delta = torch.norm(all_rot_mat_delta, dim=1)
      print(f'\033[34m [predict_pyramid] sort; to tensor time={time.time()-t}\033[0m')
      all_rot_mat_delta_idx = torch.argsort(all_rot_mat_delta, descending=False) # descending=False为从小到大；True为从大到小
      print(f'\033[34m [predict_pyramid] sort; argsort time={time.time()-t}\033[0m')
      # print(f'all_rot_mat_delta = {all_rot_mat_delta}')
      # print(f'all_rot_mat_delta_idx = {all_rot_mat_delta_idx}')
      next_level_num = min(iter_num, len(all_rot_mat_delta_idx))
      print(f'\033[34m [predict_pyramid] sort; min time={time.time()-t}\033[0m')
      B_in_cams = B_in_cams[all_rot_mat_delta_idx][:next_level_num]
      print(f'\033[34m [predict_pyramid] sort; slice time={time.time()-t}\033[0m')
      if get_vis:
        ob_centered_in_cams = B_in_cams.clone() ## TODO 
        print(f'\033[34m [predict_pyramid] sort; slice time={time.time()-t}\033[0m')
    print(f'\033[34m [predict_pyramid] iteration; total time={time.time()-start_t}\033[0m')
    # B_in_cams_out = B_in_cams@torch.tensor(tf_to_center[None], device='cuda', dtype=torch.float) ## TODO 耗时很大需要20
    B_in_cams_out = B_in_cams
    print(f'\033[34m [predict_pyramid] output; time={time.time()-start_t}\033[0m')
    # torch.cuda.empty_cache()
    self.last_trans_update = trans_delta
    self.last_rot_update = rot_mat_delta
