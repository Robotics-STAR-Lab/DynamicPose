...
class FoundationPose:
  ...
  def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
    ...
    self.correct_translation = self.get_tf_to_centered_mesh().data.cpu().numpy()
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    self.poses = poses
    self.scores = scores
    best_center = best_pose.data.cpu().numpy()[:3,3]
    if self.debug>=2:
      pcd = toOpen3dCloud(best_center.reshape(1,3))
      o3d.io.write_point_cloud(f'{self.debug_dir}/best_center.ply', pcd)
    return best_pose.data.cpu().numpy()

  # def register_with_init_pose(self, K, rgb, depth, ob_mask, init_pose, ob_id=None, glctx=None, iteration=5):
  #   set_seed(0)
  #   logging.info('Welcome')
  #   if self.glctx is None:
  #     if glctx is None:
  ...
  #   poses = torch.as_tensor([init_pose], device='cuda', dtype=torch.float)
  #   center = self.guess_translation(depth=depth, mask=ob_mask, K=K)
  #   poses[:, :3, 3] = torch.as_tensor(center.reshape(1, 3), device='cuda')
  ...
  def my_register_with_init_pose(self, K, rgb, depth, ob_mask, init_pose, ob_id=None, glctx=None, iteration=5):
    set_seed(0)
    logging.info('Welcome')
    # if self.glctx is None:
    #   if glctx is None:
    ...
    # max_idx = 0
    best_pose = poses[max_idx]@self.get_tf_to_centered_mesh()
    self.pose_last = poses[max_idx]
    
    return best_pose.cpu().numpy()
  
  def my_track_one(self, rgb, depth, K, iteration, extra={}):
    if self.pose_last is None:
      logging.info("Please init pose by register first")
      raise RuntimeError
    logging.info("Welcome")
    ...
    pose, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=self.pose_last.reshape(1,4,4).data.cpu().numpy(), normal_map=None, xyz_map=xyz_map, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=False)
    logging.info("pose done")
    # if self.debug>=2:
    #   extra['vis'] = vis
    self.pose_last = pose
  ...
  def my_track_one_with_last_pose(self, rgb, depth, K, iteration, last_o2c, extra={}):
    # self.pose_last = torch.as_tensor(last_o2c, device='cuda', dtype=torch.float)
    # last_o2c = torch.as_tensor(last_o2c, device='cuda', dtype=torch.float)
    ...
    # last_o2c =  last_o2c @ np.linalg.inv(self.correct_translation)
    pose, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=last_o2c.reshape(1,4,4), normal_map=None, xyz_map=xyz_map, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=self.debug>=2)
    logging.info("pose done")
    # if self.debug>=2:
    #   extra['vis'] = vis
    self.pose_last = pose
  ...
  def my_track_one_with_last_multiPose(self, rgb, depth, K, iteration, last_o2c_s, extra={}):
    logging.info("Welcome")
    ...
    poses = torch.as_tensor(last_o2c_s, device='cuda', dtype=torch.float)
  ...
  def coarse(self, rgb, depth, K, last_o2c_s):
    # xyz_map = depth2xyzmap_batch(depth[None], torch.as_tensor(K, dtype=torch.float, device='cuda')[None], zfar=np.inf)[0]
    t = time.time()
    poses = torch.as_tensor(last_o2c_s, device='cuda', dtype=torch.float).contiguous()
    normal_map = None
    scores, vis = self.scorer.predict(mesh=self.mesh, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(), normal_map=normal_map, mesh_tensors=self.mesh_tensors, glctx=self.glctx, mesh_diameter=self.diameter, get_vis=self.debug>=2)
    print(f'scorer.predict time: {time.time()-t}')
    # t = time.time()
    # if vis is not None:
    #   imageio.imwrite(f'{self.debug_dir}/vis_score_{self.i}.png', vis)
    #   self.i += 1
    ...
    # print(f'sort time: {time.time()-t}')
    # return best_pose[0, :3, :3].data.cpu().numpy()
    # torch.cuda.synchronize()
    out_num = min(5, len(poses))
    return poses[:out_num].to('cpu').numpy(), vis
  
  def coarse_refine_register(self, K, rgb, depth, poses, iteration=5):
      '''Copmute pose from given pts to self.pcd
      @pts: (N,3) np array, downsampled scene points
      '''
      set_seed(0)
  ...
  def track_pyramid(self, rgb, depth, K, last_o2c_s, extra={}):
    set_seed(0)
    rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
    normal_map = None
    # poses = torch.as_tensor(last_o2c_s, device='cuda', dtype=torch.float)
    poses = np.array(last_o2c_s)
    # add_errs = self.compute_add_err_to_gt_pose(poses)
    # logging.info(f"after viewpoint, add_errs min:{add_errs.min()}")

    xyz_map = depth2xyzmap(depth, K)
    # iteration = [len(last_o2c_s), min(5,len(last_o2c_s)) , 1]
    # iteration = [ ## 3层金字塔的效果才比较好
    #   min(27, len(last_o2c_s)),
    #   min(27, len(last_o2c_s)),
    #   1
    # ]
    iteration = [ ## 3层金字塔的效果才比较好
      len(last_o2c_s),
      # len(last_o2c_s),
      min(5, len(last_o2c_s)),
      # min(3, len(last_o2c_s)),
      # 1,
      1,
    ]
    poses, vis = self.refiner.predict_pyramid(
      mesh=self.mesh, 
    ...
    best_pose = poses[0]@self.get_tf_to_centered_mesh()
    self.pose_last = poses[0]
    return best_pose.cpu().numpy(), vis
